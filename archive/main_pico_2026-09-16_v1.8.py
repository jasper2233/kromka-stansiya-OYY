# main.py — KROMKA STANSIYASI KONTROLLERI  (TZ v1.2, proshivka v1.8)
#
...
#   GP0  (1-pin)   -> rele moduli IN               (sakratkich Low, lampa)
#   GP2  (4-pin)   -> QO'SHIMCHA rele 1-kanal IN    (D1 dan detal chiqqanda 1 s)
#   GP3  (5-pin)   -> QO'SHIMCHA rele 2-kanal IN    (D2 dan detal chiqqanda 1 s)
...
#   SET D=.. V10=.. V18=.. K1=.. K2=.. C=.. TD=.. LO=.. AV=.. RP=.. AO=0|1
...
#   AV     avariyada lampa/tovush yarim davri, ms
#   LO     lampa kechikishini qoplash: rele tovushdan shuncha ms OLDIN tortadi
#   RP     qo'shimcha rele kanallarining puls uzunligi, ms
...
#
# LO nima uchun kerak: LED chiroq quvvat berilgandan keyin darhol yonmaydi —
# uning drayveri sekin ko'tariladi (bizda ~1000 ms). Eski 1 s yoq / 1 s o'ch
# siklida chiroq aynan sirena TUGAGANDA yonardi — ya'ni to'liq teskari fazada.
# Endi rele LO ms oldin tortadi va sikl AV gacha uzaytirildi, shunda chiroq
# sirena bilan BIRGA ko'rinadi.
KAL = {'D': 2555.0, 'V10': 10.0, 'V18': 18.0,
       'K1': 0.0, 'K2': 0.0, 'C': 0.0, 'TD': 12.0,
       'AV': 3000.0, 'LO': 1000.0, 'RP': 1000.0, 'AO': 0.0}
...
# ========== QO'SHIMCHA 2 KANALLI RELE MODULI ==========
#
# MANTIQ: detal datchikdan CHIQQAN lahzada (nur ochilganda) o'sha datchikka
# tegishli kanal RP ms ga (standart 1 s) ulanadi va o'zi qo'yib yuboradi.
#   1-kanal  <- D1 dan detal chiqdi
#   2-kanal  <- D2 dan detal chiqdi (navbat bo'yicha juftlangan detal)
# Ikkala kanal ham bir xil ishlaydi va bir-biridan mustaqil: D1 keyingi
# detalni o'tkazayotganda D2 hali oldingisini o'tkazayotgan bo'lishi mumkin,
# shuning uchun har kanalning o'z taymeri bor.
#
# Puls BLOKLAMAYDI — asosiy sikl to'xtamaydi, o'lchov aniqligiga ta'sir qilmaydi.
GP_RELE1, GP_RELE2 = 2, 3     # 4-pin va 5-pin; orasidagi 3-pin — GND
RELE_PAST = True              # modul Low trigger bo'lsa True (mavjud rele kabi)

r_pin  = {1: Pin(GP_RELE1, Pin.OUT, value=1 if RELE_PAST else 0),
          2: Pin(GP_RELE2, Pin.OUT, value=1 if RELE_PAST else 0)}
r_bosh = {1: 0, 2: 0}         # 0 = o'chiq; aks holda puls boshlangan lahza
r_oxir = {1: None, 2: None}   # oxirgi yozilgan holat — keraksiz qo'zg'atmaslik uchun

def r_yoz(n, on):
    if r_oxir[n] is not on:
        r_oxir[n] = on
        if RELE_PAST:
            r_pin[n].value(0 if on else 1)
        else:
            r_pin[n].value(1 if on else 0)

def r_puls(n):
    """n-kanalni RP ms ga ulaydi. Puls davom etayotgan bo'lsa — qaytadan boshlaydi."""
    r_bosh[n] = time.ticks_ms() or 1      # 0 "o'chiq" degani, shuning uchun or 1
    r_yoz(n, True)

def r_tick(hozir):
    ms = int(KAL['RP'])
    for n in (1, 2):
        if r_bosh[n] and time.ticks_diff(hozir, r_bosh[n]) >= ms:
            r_bosh[n] = 0
            r_yoz(n, False)
...
def alarm_tick():
    ...
    LO = int(KAL['LO'])
    if r == 'QR':
        # Rele darhol tortadi, tovush esa LO dan keyin boshlanadi — shunda
        # sekin yonadigan chiroq va gudoklar KO'ZGA/QULOQQA birga tushadi.
        # Chiroq LO..LO+450 oralig'ida ko'rinadi, gudok ikki marta shu ichida.
        q = e - LO
        if e < LO + 450:
            lampa(True)
            tovush(1000 if (0 <= q < 150 or 300 <= q < 450) else 0)
        else:
            AL['rejim'] = 'IDLE'; lampa(False); tovush(0)
    elif r == 'TEST':
        q = e - LO
        if e < LO + 1000:
            lampa(True); tovush(1000 if q >= 0 else 0)
        else:
            AL['rejim'] = 'IDLE'; lampa(False); tovush(0)
    elif r == 'AVARIYA':
        # LAMPA va TOVUSH birga ko'rinishi kerak. Chiroq LO ms kech yonganligi
        # uchun rele shuncha OLDIN tortadi; sikl esa AV ms (chiroq ulgurishi
        # uchun 1 s dan uzun). Yongan paytda ohang 800/1200 Hz almashadi.
        AV = int(KAL['AV']) or 1000
        tov_yon = (e // AV) % 2 == 0
        lampa(tov_yon or ((e + LO) // AV) % 2 == 0)
        tovush((800 if (e // 250) % 2 == 0 else 1200) if tov_yon else 0)
...
def d1_tugadi(us, ms, dur):
    ...
    r_puls(1)                    # 1-kanal: detal D1 dan chiqdi
...
def d2_tugadi(us, dur):
    ...
    r_puls(2)                    # 2-kanal: detal D2 dan chiqdi
    if kutuv:
...
def buyruq_tekshir():
    ...
    if cmd == 'QR':
        ...
        yubor({'ev': 'kal', 'D': KAL['D'], 'V10': KAL['V10'], 'V18': KAL['V18'],
               ...
               'AV': KAL['AV'], 'LO': KAL['LO'], 'RP': KAL['RP'], 'AO': KAL['AO']})
        chop("  kalibrlash saqlandi: D=%.2f V10=%.3f V18=%.3f K1=%.3f K2=%.3f C=%.3f TD=%.1f"
             % (KAL['D'], KAL['V10'], KAL['V18'], KAL['K1'], KAL['K2'], KAL['C'], KAL['TD']))
        chop("  signal: LO=%d AV=%d RP=%d  avariya=%s"
             % (KAL['LO'], KAL['AV'], KAL['RP'], "O'CHIQ" if KAL['AO'] else 'yoniq'))
    elif cmd == 'PING':
...
if CHOP:
    print("=" * 50)
    print("  KROMKA STANSIYASI KONTROLLERI  v1.8")
    print("  D1=GP%d  D2=GP%d  RUN=GP%d  V18=GP%d  RELE=GP%d  AUDIO=GP%d"
          % (GP_D1, GP_D2, GP_RUN, GP_V18, GP_RELE, GP_AUDIO))
    print("  Qo'shimcha rele: 1-kanal=GP%d (4-pin), 2-kanal=GP%d (5-pin), puls %d ms"
          % (GP_RELE1, GP_RELE2, int(KAL['RP'])))
...
while True:
    ...
    alarm_tick()
    r_tick(hozir)                     # qo'shimcha rele pulslarini qo'yib yuborish
